...
import { useSeatMap, useCreateHold, useConfirmBooking } from '../lib/bookingQueries'
...
export default function SeatSelectionPage() {
  ...
  const { data: seatMap, isLoading, error, refetch } = useSeatMap(id)
  ...
  const gridSeats: GridSeat[] = seatMap?.seats.map((s) => ({ id: s.showSeatId, ...s })) ?? []
  const selectedSeats = seatMap?.seats.filter((s) => selected.has(s.showSeatId)) ?? []
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-neutral-950 px-6 py-8 text-neutral-100">
        <p className="text-red-400">Could not load this show.</p>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-neutral-950 pb-28 text-neutral-100">
      <header className="sticky top-0 z-40 border-b border-neutral-800 bg-neutral-950/80 px-6 py-4 backdrop-blur">
        ...
        <h1 className="mt-2 text-lg font-semibold">{seatMap.movieTitle}</h1>
